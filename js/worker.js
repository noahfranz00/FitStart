...
// Serves: index.html, styles.css, app.js, scoring.js, logger.js, coach.js
// ═══════════════════════════════════════════
      ...
      <div class="prog-card" style="margin-bottom:14px">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
          ...
          <div style="display:flex;align-items:center;gap:8px">
            ...
            <button onclick="logBodyWeight()" style="padding:8px 14px;background:var(--gold-grad);border:none;border-radius:8px;color:var(--black);font-family:'Bebas Neue',sans-serif;font-size:0.85rem;letter-spacing:1px;cursor:pointer">LOG</button>
          </div>
        </div>
...
<div id="unplanned-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.92);z-index:6000;align-items:flex-end;justify-content:center">
  <div id="unplanned-modal-inner" style="background:var(--dark);border:1px solid var(--border2);border-radius:24px 24px 0 0;width:100%;max-width:600px;padding:24px 20px 40px;max-height:92dvh;overflow-y:auto;transition:max-height 0.3s ease">
    ...
    <div id="ai-workout-preview" style="display:none">
      ...
      <button onclick="launchAIWorkout()" style="width:100%;padding:16px;background:var(--gold-grad);border:none;border-radius:12px;color:var(--black);font-family:'Bebas Neue',sans-serif;font-size:1.15rem;letter-spacing:2px;cursor:pointer">BEGIN WORKOUT →</button>
    ...
    <div id="custom-builder" style="display:none;margin-top:4px">
      ...
      <div id="custom-selected" style="margin-top:16px;display:none">
        ...
        <button onclick="launchCustomWorkout()" style="width:100%;padding:16px;background:var(--gold-grad);border:none;border-radius:12px;color:var(--black);font-family:'Bebas Neue',sans-serif;font-size:1.1rem;letter-spacing:2px;cursor:pointer">BEGIN WORKOUT →</button>
      </div>
    </div>
  </div>
</div>
  ...
  <div style="padding:20px;border-bottom:1px solid var(--border)">
    ...
    <div style="display:flex;gap:10px">
      ...
      <button onclick="submitBarcode()"
        style="padding:14px 24px;background:var(--gold-grad);border:none;border-radius:10px;color:var(--black);font-family:'Bebas Neue',sans-serif;font-size:1rem;letter-spacing:1.5px;cursor:pointer;font-weight:700;white-space:nowrap">LOOK UP</button>
    </div>
...
<div class="add-ex-modal" id="dash-ex-picker-modal">
  <div class="add-ex-sheet">
    ...
    <div id="aes-build-tray" style="display:none;border-top:1px solid var(--border);padding:14px 16px;background:var(--dark)">
      ...
      <button onclick="beginCustomWorkoutFromPicker()" id="aes-begin-btn" style="display:none;width:100%;padding:13px;background:var(--gold-grad);border:none;border-radius:10px;color:var(--black);font-family:'Bebas Neue',sans-serif;font-size:1rem;letter-spacing:2px;cursor:pointer">BEGIN WORKOUT →</button>
    </div>
    ...
    <div id="aes-config-overlay" style="display:none;position:absolute;inset:0;background:rgba(0,0,0,0.92);border-radius:inherit;z-index:10;flex-direction:column;align-items:center;justify-content:center;padding:28px 24px">
      <div style="width:100%;max-width:320px">
        ...
        <div style="display:flex;gap:10px">
          ...
          <button onclick="confirmAesConfig()" style="flex:2;padding:13px;background:var(--gold-grad);border:none;border-radius:10px;color:var(--black);font-family:'Bebas Neue',sans-serif;font-size:1rem;letter-spacing:2px;cursor:pointer">ADD TO WORKOUT</button>
        </div>
      </div>
    </div>
  </div>
...
<div id="setrepedit-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.82);z-index:9500;align-items:center;justify-content:center;padding:20px" onclick="if(event.target===this)closeSetRepEditor()">
  <div style="background:var(--card);border:1px solid var(--border);border-radius:18px;padding:24px 22px;width:100%;max-width:340px" onclick="event.stopPropagation()">
    ...
    <button onclick="applySetRepEdit()" style="width:100%;padding:13px;background:var(--gold-grad);border:none;border-radius:10px;color:var(--black);font-family:'Bebas Neue',sans-serif;font-size:1rem;letter-spacing:2px;cursor:pointer">APPLY</button>
  </div>
</div>
...
.line-accent { background:var(--gold-grad); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; transition:color 0.4s; display:block; }
...
.btn-generate { width:100%; margin-top:24px; padding:16px; background:var(--gold-grad); color:var(--black); border:none; border-radius:12px; font-family:'Bebas Neue',sans-serif; font-size:1.1rem; letter-spacing:2px; cursor:pointer; transition:all 0.2s; font-weight:700; }
...
.sc-val { font-family:'Bebas Neue',sans-serif; font-size:1.8rem; letter-spacing:1px; background:var(--gold-grad); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; line-height:1; margin-bottom:3px; transition:color 0.3s; }
.sc-val.over { background:var(--red); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; }
...
.prog-fill { height:100%; border-radius:2px; background:var(--gold-grad); transition:width 0.4s ease, background 0.3s; }
...
.btn-primary { padding:11px 24px; background:var(--gold-grad); color:var(--black); border:none; border-radius:10px; font-family:'Bebas Neue',sans-serif; font-size:0.95rem; letter-spacing:2px; cursor:pointer; transition:all 0.2s; flex-shrink:0; font-weight:700; }
...
.mb-fill { height:100%; border-radius:2px; background:var(--gold); transition:width 0.4s ease, background 0.3s; }
...
.btn-cat-add { display:flex; align-items:center; gap:5px; padding:7px 13px; background:var(--gold-dim); border:1px solid rgba(212,165,32,0.2); border-radius:8px; color:var(--gold); font-family:'Bebas Neue',sans-serif; font-size:0.78rem; letter-spacing:1px; cursor:pointer; white-space:nowrap; transition:all 0.2s; }
.btn-cat-add:hover { background:rgba(212,165,32,0.2); }
