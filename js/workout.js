...
function openWorkoutEnv(dayIdx, startExIdx) {
  ...
  _warmChimeAudio(); // unlock audio once, in user gesture context (Start Workout tap)
  renderEcList();
...
function loadExercise(idx) {
  ...
  // Restore video section visibility (reset when switching exercises)
  var demoWrap = document.getElementById('wo-demo-wrap');
  var showBtn = document.getElementById('wo-video-show');
  if (demoWrap) demoWrap.style.display = '';
  ...
  // Rest timer — don't kill a running timer (user may have minimized/resumed)
...
function dismissExVideo() {
  var wrap = document.getElementById('wo-demo-wrap');
  if (wrap) wrap.style.display = 'none';
  // Create or show "Show Video" button
  var btn = document.getElementById('wo-video-show');
  if (!btn) {
    btn = document.createElement('button');
    btn.id = 'wo-video-show';
    btn.onclick = function() { showExVideo(); };
    btn.style.cssText = 'display:flex;align-items:center;gap:6px;margin:0 14px 8px;padding:8px 14px;background:none;border:1px solid rgba(255,255,255,0.08);border-radius:8px;color:#6B6865;font-size:0.72rem;cursor:pointer;font-family:inherit';
    btn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="14" height="14"><polygon points="6 3 20 12 6 21"/></svg> Show Video';
    // Insert before the sets panel
    var setsPanel = document.querySelector('.sets-panel');
    if (setsPanel && setsPanel.parentNode) setsPanel.parentNode.insertBefore(btn, setsPanel);
  }
  btn.style.display = 'flex';
}

function showExVideo() {
  var wrap = document.getElementById('wo-demo-wrap');
...
// Keep audio authorized on iOS — silent play/pause on explicit user gesture in workout env only
var _isWarming = false;
function _warmChimeAudio() {
  _initChimeAudio();
  if (!_chimeAudio || _isWarming) return;
  _isWarming = true;
  try {
    ...
    if (p && p.then) {
      p.then(function() {
        try { _chimeAudio.pause(); _chimeAudio.currentTime = 0; } catch(e) {}
        _chimeAudio.volume = 1.0;
        _isWarming = false;
      }).catch(function() {
        _chimeAudio.volume = 1.0;
  ...
  } catch(e) { _chimeAudio.volume = 1.0; _isWarming = false; }
}

// DO NOT attach global touchstart/click warm listeners — they interrupt background
// music on iOS by stealing audio session focus on every tap.
// The audio context is warmed once when the workout environment opens.
...
var restTimerEndAt = 0;
var _chimePlayedForCurrentTimer = true; // true = armed against accidental fires

function buildRestSelect() {}

function updateRestDisplay() {
  var lbl = document.getElementById('rest-timer-label');
  var secs = document.getElementById('rest-timer-secs-display');
...
function toggleRestTimer() {
  if (restTimerInterval) skipRest();
  else startRestTimerSecs(restTotalSecs);
}
...
function _startTimerUI(totalSecs) {
  var label = document.getElementById('rest-timer-label');
  var secsDisplay = document.getElementById('rest-timer-secs-display');
  var bar = document.getElementById('rest-timer-progress');
  if (label) label.textContent = 'TAP TO STOP';
  if (bar) { bar.style.transition = 'none'; bar.style.width = '0%'; }
  if (navigator.vibrate) navigator.vibrate(50);

  restTimerInterval = setInterval(function() {
    var remaining = Math.ceil((restTimerEndAt - Date.now()) / 1000);
    if (secsDisplay) secsDisplay.textContent = Math.max(0, remaining) + 's';
    if (bar) {
      var elapsed = totalSecs - remaining;
      var pct = Math.min(100, (elapsed / totalSecs) * 100);
      bar.style.transition = 'width 0.5s linear';
      bar.style.width = pct + '%';
    }
    if (remaining <= 0 && !_chimePlayedForCurrentTimer) {
...
function skipRest() {
  _chimePlayedForCurrentTimer = true; // arm guard before clearing interval to prevent phantom tick
...
// Restore timer — NO CHIME from here, ever. Only visual update.
function restoreRestTimerIfActive() {
  // Already running? Just refresh the display.
  if (restTimerInterval && restTimerEndAt > 0) {
    var remaining = Math.ceil((restTimerEndAt - Date.now()) / 1000);
    var secsDisplay = document.getElementById('rest-timer-secs-display');
    if (secsDisplay && remaining > 0) secsDisplay.textContent = remaining + 's';
    return;
  }
  ...
  _initChimeAudio();
  // Double-check: re-compute remaining right before arming, in case time slipped
  var remainingFinal = Math.ceil((restTimerEndAt - Date.now()) / 1000);
  if (remainingFinal <= 0) {
    lsSet('fs_rest_timer', null);
    _chimePlayedForCurrentTimer = true; // stay armed
    return;
  }
  _chimePlayedForCurrentTimer = false; // arm ONLY when we're certain timer is live
  _startTimerUI(restTotalSecs);
}

// Visibility change — display update only, no chime
document.addEventListener('visibilitychange', function() {
  if (document.visibilityState === 'visible' && restTimerInterval && restTimerEndAt > 0) {
    ...
    var bar = document.getElementById('rest-timer-progress');
    if (secsDisplay) secsDisplay.textContent = Math.max(0, remaining) + 's';
    if (remaining > 0 && bar && restTotalSecs > 0) {
      var pct = Math.min(100, ((restTotalSecs - remaining) / restTotalSecs) * 100);
      bar.style.width = pct + '%';
    }
  }
});
