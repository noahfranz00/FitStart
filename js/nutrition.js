...
// ═══════════════════════════════════════════
// WHAT SHOULD I EAT? — AI meal suggestions based on remaining macros
// ═══════════════════════════════════════════
function askWhatToEat() {
  const panel = document.getElementById('meal-suggest-panel');
  if (!panel) return;

  // Calculate remaining macros for today
  const logged = getAllEntries(TODAY_IDX).reduce(function(a, e) {
    return { cal: a.cal + e.cal, pro: a.pro + (e.pro||0), carb: a.carb + (e.carb||0), fat: a.fat + (e.fat||0) };
  }, { cal: 0, pro: 0, carb: 0, fat: 0 });

  const rem = {
    cal:  Math.max(0, TARGETS.cal  - logged.cal),
    pro:  Math.max(0, TARGETS.pro  - logged.pro),
    carb: Math.max(0, TARGETS.carb - logged.carb),
    fat:  Math.max(0, TARGETS.fat  - logged.fat)
  };

  const barEl = document.getElementById('ms-remaining-bar');
  if (barEl) {
    barEl.innerHTML = [
      `<span style="color:var(--bone)">Remaining:</span>`,
      `<span style="margin-left:10px">⚡ <b>${rem.cal}</b> cal</span>`,
      `<span style="margin-left:10px">💪 <b>${rem.pro}g</b> protein</span>`,
      `<span style="margin-left:10px">🌾 <b>${rem.carb}g</b> carbs</span>`,
      `<span style="margin-left:10px">🥑 <b>${rem.fat}g</b> fat</span>`
    ].join('');
  }

  const contentEl = document.getElementById('ms-content');
  if (contentEl) contentEl.innerHTML = '<div style="text-align:center;padding:30px;color:var(--dim);font-size:0.8rem"><div style="margin-bottom:8px;font-size:1.2rem">✨</div>Generating suggestions...</div>';

  panel.style.display = 'flex';

  // Build prompt
  const userName = (USER && USER.name) ? USER.name.split(' ')[0] : 'you';
  const now = new Date();
  const hour = now.getHours();
  const timeOfDay = hour < 11 ? 'morning' : hour < 14 ? 'midday' : hour < 18 ? 'afternoon' : 'evening';
  const injuries = USER && USER.injuries ? 'Dietary restrictions / notes: ' + USER.injuries + '.' : '';

  const prompt = `You are a precision nutrition coach. The user has these macros REMAINING for today (${timeOfDay}):
- Calories: ${rem.cal} kcal
- Protein: ${rem.pro}g
- Carbs: ${rem.carb}g  
- Fat: ${rem.fat}g
...
Generate EXACTLY 3 specific meal suggestions that fit within these remaining macros. Each suggestion should be realistic, practical, and specifically portioned to match.

Respond ONLY with a JSON array (no markdown, no preamble) in this exact format:
[
  {
    "name": "Meal name (specific, with portion size)",
    "cal": 420,
    "pro": 38,
    "carb": 32,
    "fat": 14,
    "desc": "One sentence describing why this fits perfectly right now and how to prepare it quickly.",
    "ingredients": "Specific ingredients with quantities"
  }
]

Rules:
- Each meal MUST fit within the remaining macros (not exceed them)
- Be specific: "6oz grilled chicken + 3/4 cup brown rice + 1 cup broccoli" not just "chicken and rice"
- Consider the time of day (${timeOfDay}) — lighter if evening/already ate a lot today
- Prioritize hitting the protein target
- Numbers must be realistic and accurate`;

  if (typeof callClaude !== 'function') {
    if (contentEl) contentEl.innerHTML = '<div style="color:var(--dim);text-align:center;padding:20px">Coach not available.</div>';
    return;
  }

  callClaude([{ role: 'user', content: prompt }], { max_tokens: 800 })
    .then(function(reply) {
      let meals = [];
      try {
        ...
        if (match) {
          try { meals = JSON.parse(match[0]); } catch(e2) {}
        }
      }
      if (!meals || !meals.length) {
        if (contentEl) contentEl.innerHTML = '<div style="color:var(--dim);text-align:center;padding:20px">Couldn\'t generate suggestions right now. Try the coach directly.</div>';
        return;
      }
      if (contentEl) {
        contentEl.innerHTML = meals.map(function(m) {
          const macroStr = `${m.cal} cal · ${m.pro}g P · ${m.carb}g C · ${m.fat}g F`;
          const encodedMsg = encodeURIComponent('I want to eat: ' + m.name + '. That\'s ' + m.ingredients + '. Macros: ' + macroStr + '. Please log this for me.');
          return `<div class="ms-meal-card" onclick="closeMealSuggestPanel();_openCoachWithMealSuggestion(${JSON.stringify(m).replace(/"/g,'&quot;')})">
            <div class="ms-meal-name">${m.name}</div>
            <div class="ms-meal-macros">${macroStr}</div>
            <div class="ms-meal-desc">${m.desc || ''}</div>
            <div style="margin-top:8px;font-size:0.7rem;color:var(--dim);font-family:'DM Mono',monospace">${m.ingredients || ''}</div>
          </div>`;
        }).join('');
      }
    })
    .catch(function(e) {
      if (contentEl) contentEl.innerHTML = '<div style="color:var(--dim);text-align:center;padding:20px">Something went wrong. Try again.</div>';
    });
}

function closeMealSuggestPanel() {
  var panel = document.getElementById('meal-suggest-panel');
  if (panel) panel.style.display = 'none';
}

function _openCoachWithMealSuggestion(meal) {
  // Navigate to coach and pre-fill with meal logging message
  if (typeof dashNav === 'function') {
    const btn = document.querySelector('.sb-btn[onclick*="coach"]') || document.querySelector('[onclick*="coach"]');
    dashNav('coach', btn);
  }
  // Pre-fill the coach input with the meal details
  setTimeout(function() {
    const input = document.getElementById('coach-input');
    const macroStr = meal.cal + ' cal, ' + meal.pro + 'g protein, ' + meal.carb + 'g carbs, ' + meal.fat + 'g fat';
    if (input) {
      input.value = 'Log this for me: ' + meal.name + ' — ' + meal.ingredients + ' (' + macroStr + ')';
      input.focus();
    }
  }, 300);
}
...
// ═══════════════════════════════════════════
// SCAN PLATE — Photo macro estimation & logging
// ═══════════════════════════════════════════
var _scanPlateImage = null; // { base64, mediaType }

function openScanPlateModal() {
  _scanPlateImage = null;
  var modal = document.getElementById('scan-plate-modal');
  if (!modal) return;
  // Reset UI
  document.getElementById('sp-preview-wrap').style.display = 'none';
  document.getElementById('sp-photo-btns').style.display = 'flex';
  document.getElementById('sp-desc').value = '';
  document.getElementById('sp-submit-btn').disabled = false;
  document.getElementById('sp-submit-btn').textContent = 'ANALYZE & LOG';
  // Set meal category to current meal time
  const hour = new Date().getHours();
  const cat = document.getElementById('sp-meal-cat');
  if (cat) cat.value = hour < 10 ? 'breakfast' : hour < 14 ? 'lunch' : hour < 18 ? 'dinner' : 'snacks';
  modal.style.display = 'flex';
}

function closeScanPlateModal() {
  var modal = document.getElementById('scan-plate-modal');
  if (modal) modal.style.display = 'none';
  _scanPlateImage = null;
}

function handleScanPlatePhoto(input, source) {
  const file = input.files && input.files[0];
  if (!file) return;
  if (file.size > 5 * 1024 * 1024) {
    showToast('Image too large — please use one under 5MB.', 'warning');
    input.value = '';
    return;
  }
  ...
  reader.onload = function(e) {
    ...
    _scanPlateImage = { base64: dataUrl.split(',')[1], mediaType: file.type || 'image/jpeg' };
    // Show preview
    const wrap = document.getElementById('sp-preview-wrap');
    const img = document.getElementById('sp-preview-img');
    if (wrap && img) { img.src = dataUrl; wrap.style.display = 'flex'; }
    document.getElementById('sp-photo-btns').style.display = 'none';
    // Add a "retake" link below preview
    const retake = document.getElementById('sp-retake-link');
    if (!retake) {
      ...
      a.style.cssText = 'background:none;border:none;color:var(--dim);font-size:0.72rem;cursor:pointer;margin-bottom:10px;text-decoration:underline;font-family:inherit';
      a.textContent = 'Remove photo';
...
async function submitScanPlate() {
  if (!_scanPlateImage && !document.getElementById('sp-desc').value.trim()) {
    showToast('Add a photo or describe your meal first.', 'warning');
    return;
  }
  const desc = document.getElementById('sp-desc').value.trim();
  const mealCat = document.getElementById('sp-meal-cat').value || 'other';
  const btn = document.getElementById('sp-submit-btn');
  btn.disabled = true;
  btn.textContent = 'ANALYZING...';

  try {
    // Build message content — image + text description
    let userContent;
    const textMsg = desc
      ? `Here is a photo of my meal. I'm eating: ${desc}. Please estimate the macros as accurately as possible and log it to my ${mealCat}.`
      : `Here is a photo of my meal. Please identify what's on the plate, estimate portions and macros, and log it to my ${mealCat}.`;

    if (_scanPlateImage) {
      userContent = [
        { type: 'image', source: { type: 'base64', media_type: _scanPlateImage.mediaType, data: _scanPlateImage.base64 } },
    ...
    const systemPrompt = `You are a nutrition coach analyzing a meal photo to log macros.
Estimate calories, protein, carbs, and fat as accurately as possible based on what you can see and the user's description.
Be specific about portions — default to realistic home-cooked serving sizes.
Always include a FOODLOG block at the END of your response to log the meal.

FOODLOG FORMAT (must be at the very end of your response):
\`\`\`FOODLOG
[
  { "name": "Food item with portion", "category": "${mealCat}", "cal": 0, "pro": 0, "carb": 0, "fat": 0 }
]
\`\`\`

Show the macro breakdown clearly in your response: **X cal · Xg protein · Xg carbs · Xg fat**`;

    window._pendingMealCat = mealCat;
    window._pendingMealLabel = mealCat.charAt(0).toUpperCase() + mealCat.slice(1);

    // Send through coach system so FOODLOG is processed
    if (typeof _sendCoachMsg === 'function') {
      closeScanPlateModal();
      // Navigate to coach
      const coachBtn = document.querySelector('.sb-btn[onclick*="coach"]') || document.querySelector('[onclick*="coach"]');
      if (typeof dashNav === 'function') dashNav('coach', coachBtn);
      // Use coach's image pipeline
      if (_scanPlateImage) {
        window._coachPendingImage = _scanPlateImage;
        _scanPlateImage = null;
      }
      setTimeout(function() {
        _sendCoachMsg(textMsg, window._coachPendingImage);
        window._coachPendingImage = null;
      }, 200);
    } else {
      // Fallback: direct callClaude
      const reply = await callClaude([{ role: 'user', content: userContent }], {
        system: systemPrompt,
        max_tokens: 600
      });
      closeScanPlateModal();
      // Parse FOODLOG
      if (typeof parseFoodLogBlock === 'function') {
        parseFoodLogBlock(reply);
      }
      showToast('Meal analyzed and logged!', 'success');
    }
  } catch(e) {
    btn.disabled = false;
    btn.textContent = 'ANALYZE & LOG';
    showToast('Could not analyze meal. Try again.', 'error');
  }
}
